...
public class MarkInfoDialog extends Dialog {
    ...
    private void setData(ViewHolder holder) {
        DiaryAPI.startAsyncTask(() -> {
            List<Mark> workMarks = StaticRecourses.diaryAPI.getMarksByWork(mark.getWork());
                ...
                for (Person person : personList) {
                    if (mark.getPerson().equals(person.getId())) {
                        if (!personMarkMap.containsKey(person)) {
                            ...
                            List<Mark> markList = personMarkMap.get(person) == null ?
                                    new ArrayList<>() : personMarkMap.get(person);
                            markList.add(mark);
                            personMarkMap.put(person, markList);
                        }
                    }
            ...
            for (String markVal : marksValueMap.keySet()) {
                switch (marksValueMap.get(markVal).get(0).getMood()) {
                    case Mood.GOOD:
            ...
            for (List<Mark> markList : colorMap.keySet()) {
                if (markList.size() != 0) {
                    ...
                    dataSet.addColor(colorMap.get(markList));
                    entries.add(entry);
                }
            }

            Log.d("main", entries.toString());
